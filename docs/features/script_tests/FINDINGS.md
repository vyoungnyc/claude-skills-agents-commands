...
- **Expected:** the comm-match should identify the specific `run-tests.sh` invocation, not any bash process — `*run-tests*` alone (already present as one arm of the pattern) is sufficient; `*bash*` should be dropped rather than OR'd in, since `bash scripts/run-tests.sh` reports `comm=bash` on some platforms while `run-tests.sh` executed directly (with a shebang) reports `comm=run-tests.sh`, so both arms currently exist to cover both invocation styles — but the `*bash*` arm is far broader than necessary for that purpose.
...
- **Disposition:** out of scope for this feature per REQ-013 (would require editing `hooks/auto-test-runner.sh`, not the four scripts under test). Recommend a follow-up ticket: narrow the comm-match to `*run-tests*` only.

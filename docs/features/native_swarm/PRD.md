...
- REQ-004: **Retire `scripts/swarm-dispatch.sh`.** Delete the script and remove/replace every live reference.
  - AC: `grep -rn "swarm-dispatch" --include="*.md" --include="*.sh"` returns only: CHANGELOG history entries, this PRD, and `docs/PHASE_6_NATIVE_PARALLELISM.md` (the decision record). Any other hit is a miss to fix.

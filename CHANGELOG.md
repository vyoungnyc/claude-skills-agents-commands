...
- **Deploy the shell test runner at the hook's resolved path (Codex PR #38 P2):** the copied `hooks/scripts/auto-test-runner.sh` resolves its shell-suite runner as a `run-tests.sh` sibling in `hooks/scripts/`, but the hooks loop only copied `hooks/*.sh` — so on a clean deploy every `.sh` edit hit `[ -f "$RUN_TESTS" ] || exit 0` and the advertised test hook silently no-opped. The converter now copies `scripts/run-tests.sh` to that resolved path; new test case (4) assertion covers it.

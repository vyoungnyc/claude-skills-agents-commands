...
- **Replace a conflicting directory before deploying a staged file (Codex PR #38):** if a live target holds a directory where a staged file belongs (e.g. `agent/agents/reviewer.md/`), a plain `cp` nested the file inside it (`reviewer.md/reviewer.md`) — the apply "succeeded" but omp couldn't find the agent and every later sync re-reported the change. The overlay now detects a non-regular destination and replaces it (the whole-dir snapshot already preserved it) before copying.

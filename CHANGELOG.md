...
- **Replace a conflicting directory or symlink before deploying a staged file (Codex PR #38, both scripts):** if a live target holds a directory where a staged file belongs (e.g. `agent/agents/reviewer.md/`), a plain `cp` nested the file inside it (`reviewer.md/reviewer.md`); if it holds a symlink, `cp` followed the link and overwrote the EXTERNAL referent (which the snapshot then read as the new bytes). Either way the apply "succeeded" but left the wrong thing at the path. Both overlays now detect a non-regular/symlink destination and remove it (the whole-dir snapshot already preserved it) before copying — the omp overlay per file, the Claude dir overlay via a pre-copy symlink sweep plus the per-file hooks path. The omp overlay also handles a **symlinked category root** (e.g. `agent/agents` itself a symlink): it records the link in the backup and replaces it with a real directory before iterating, so the deploy writes into the intended path and the external referent is left untouched.
